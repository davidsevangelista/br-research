...
def load_lob_zip(path,symbol,date,depth):
    ...
    if(Path(zip_name).is_file()):
        ...
        if(file in [i.filename for i in zip.filelist]):
            ...
            offer_price.columns = ['Time Frame', 'Report Time'] + ['price' + str(i) for i in range(0,depth)]
            offer_price = offer_price[offer_price['price0'] != '']
            for i in range(0,depth):
                offer_price['price' + str(i)] = pd.to_numeric(offer_price['price' + str(i)], errors='coerce')
            f.close()
        else:
            offer_price = []
        ...
        if (file in [i.filename for i in zip.filelist]):
            ...
            bid_price = pd.DataFrame(
                [data[i][0:min(depth + 2, len(data[i]))] if len(data[i]) >= 3 else [] for i in
                 range(0, len(data))])
            bid_price.columns = ['Time Frame', 'Report Time'] + ['price' + str(i) for i in range(0, depth)]
            bid_price = bid_price[bid_price['price0'] != '']
            for i in range(0, depth):
                bid_price['price' + str(i)] = pd.to_numeric(bid_price['price' + str(i)], errors='coerce')
            f.close()
        else:
            bid_price = []
        ...
        if (file in [i.filename for i in zip.filelist]):
            ...
            offer_qty = pd.DataFrame(
                [data[i][0:min(depth + 2, len(data[i]))] if len(data[i]) >= 3 else [] for i in
                 range(0, len(data))])
            offer_qty.columns = ['Time Frame', 'Report Time'] + ['qty' + str(i) for i in range(0, depth)]
            offer_qty = offer_qty[offer_qty['qty0'] != '']
            for i in range(0, depth):
                offer_qty['qty' + str(i)] = pd.to_numeric(offer_qty['qty' + str(i)], errors='coerce')
            f.close()
        else:
            offer_qty = []
        ...
        if (file in [i.filename for i in zip.filelist]):
            ...
            bid_qty = pd.DataFrame(
                [data[i][0:min(depth + 2, len(data[i]))] if len(data[i]) >= 3 else [] for i in
                 range(0, len(data))])
            bid_qty.columns = ['Time Frame', 'Report Time'] + ['qty' + str(i) for i in range(0, depth)]
            bid_qty = bid_qty[bid_qty['qty0'] != '']
            for i in range(0, depth):
                bid_qty['qty' + str(i)] = pd.to_numeric(bid_qty['qty' + str(i)], errors='coerce')
            f.close()
        else:
            bid_qty = []

    return {'Bid Price': bid_price, 'Ask Price': offer_price, 'Bid Volume':bid_qty, 'Ask Volume':offer_qty}
